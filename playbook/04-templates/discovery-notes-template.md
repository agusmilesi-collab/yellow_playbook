# Plantilla — Notas de Discovery Call

**SSOT** del formato de notas que se generan después de cada discovery call.

Para el proceso de la call, ver `../03-process/01-discovery.md`.

---

## Estructura obligatoria

```markdown
# Discovery Call — [Nombre del prospect]

**Fecha:** [Fecha]
**Duración real:** [X minutos]
**Canal:** [Zoom / Google Meet / presencial]

---

## Datos del prospect

- **Nombre:** [Nombre completo]
- **Negocio:** [Qué hace]
- **Antigüedad:** [X años de oficio]
- **Tamaño:** [Solo / equipo de X]
- **Contacto:** [WhatsApp / email]
- **Cómo llegó:** [Referido por X / LinkedIn / Otro]

---

## Pregunta 1 — Contame tu negocio

[Notas. Frases textuales entre comillas. NO parafrasear todo.]

**Frases clave:**

> "[frase textual del cliente]"

> "[otra frase textual]"

---

## Pregunta 2 — ¿Qué te está costando hoy?

[Notas. Foco en struggling moments.]

**Síntomas que nombró:**
- [Síntoma 1]
- [Síntoma 2]

**Frases clave:**

> "[frase textual]"

---

## Pregunta 3 — ¿Qué pasaría si no se resuelve?

[Notas. Foco en push y costo de no actuar.]

**Lectura del push:** [Alto / Medio / Bajo]

**Frases clave:**

> "[frase textual]"

---

## Pregunta 4 — Si pudieras chasquear los dedos

[Notas. Foco en transformación y los 3 jobs.]

**Frases clave:**

> "[frase textual]"

---

## Hipótesis preliminares (🔵)

### Job dominante

> *"Cuando [SITUACIÓN], quiere [MOTIVACIÓN], para poder [RESULTADO ESPERADO]."*

### Los 3 jobs

- **Functional 🔵:** [hipótesis]
- **Emotional 🔵:** [hipótesis]
- **Social 🔵:** [hipótesis]

### Struggling moment principal

[Hipótesis del momento de frustración más fuerte]

### Contra qué compite (hipótesis)

- [Alternativa 1]
- [Alternativa 2]

---

## Calificación contra ICP

Ver criterios en `../01-identity/icp.md`.

| Dimensión | Score (1-5) | Notas |
|---|---|---|
| Oficio sólido | [X] | [Por qué] |
| Capacidad de pago | [X] | [Por qué] |
| Problema bien diagnosticado | [X] | [Por qué] |
| Velocidad de decisión | [X] | [Por qué] |
| Apertura al feedback | [X] | [Por qué] |
| **TOTAL** | **[X/25]** | |

**Lectura:**
- 20-25: cliente ideal, prioridad alta.
- 14-19: buen fit, avanzar.
- <14: declinar.

---

## Red flags detectadas

[Marcar las que aparecieron, si alguna. Ver `../01-identity/icp.md`.]

- [ ] Preguntó precio antes que problema
- [ ] Quiere "más clientes" / marketing
- [ ] Quiere ejecución, no estrategia
- [ ] Pidió rebaja de entrada
- [ ] Comité de decisión grande
- [ ] Tono dominante / cerrado
- [ ] Otra: [especificar]

## Green flags detectadas

- [ ] Articula bien su problema
- [ ] Reconoce que no sabe la solución
- [ ] Preguntas inteligentes sobre metodología
- [ ] Habla de sus clientes con respeto
- [ ] Pregunta proceso antes que precio
- [ ] Decisor único o con socio único
- [ ] Otra: [especificar]

---

## Modelo recomendado

[A / B / C]

**Razón:** [Por qué este modelo y no otro]

---

## Gaps de información (❓)

[Qué falta saber antes de armar propuesta sólida]

- [ ] [Gap 1]
- [ ] [Gap 2]

---

## Decisión

- [ ] **Avanzar con propuesta** — fecha de envío: [Fecha]
- [ ] **Declinar cordialmente** — razón: [Motivo]
- [ ] **Pedir más información** — qué: [Qué falta]

---

## Próxima acción

[Acción concreta, fecha, responsable]
```

---

## Notas para la IA

Cuando recibas notas crudas de una discovery call, generar un documento siguiendo esta plantilla. Reglas:

1. **Estructurar por las 4 preguntas** core.
2. **Capturar frases textuales** del cliente (entre comillas).
3. **Calificar contra ICP** siguiendo el sistema de scoring.
4. **Hipotetizar los 3 jobs** con marcador 🔵.
5. **Identificar struggling moment** principal.
6. **Marcar gaps** con marcador ❓.
7. **Recomendar modelo** según signals.
8. **NO armar propuesta todavía.** Solo estructurar.

---

## Changelog del archivo

- **Mayo 2026:** versión inicial v0.2.
